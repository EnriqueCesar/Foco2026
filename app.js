...
const metricDefs = {
  ...
  cx:     {name:'Conexión', short:'Conexión', fmt:'pct1', dir:'up', threshold:.60},
  bebida: {name:'Calidad de Bebidas', short:'Bebida', fmt:'pct1', dir:'up', threshold:.71},
  ...
  costo:  {name:'Variación de Inventario', short:'Inventario', fmt:'pct1', dir:'down', threshold:.009},
  ctc:    {name:'Cada Taza Cuenta', short:'CTC', fmt:'pct1', dir:'up', threshold:.10}
};
const pillars = [
...
function fillStoreAll(){
  ...
  $('storeList').innerHTML = sorted.map(s =>
    `<option value="${escapeHtml(s.tienda)}">${s.ceco} · ${escapeHtml(s.tienda)}</option><option value="${s.ceco}">${s.ceco} · ${escapeHtml(s.tienda)}</option><option value="${s.ceco} · ${escapeHtml(s.tienda)}"></option>`
  ).join('');
}
...
function weeks(){ return (monthWeeks5[$('mes').value] || (D.monthWeeks && D.monthWeeks[$('mes').value]) || []).slice(0,5); }
...
function bestWorst(cecos){
  ...
  return {best: metricDefs[items[0].k].short, worst: metricDefs[items[items.length-1].k].short};
}
function trendOverall(cecos){
  ...
  const d = avg(deltas);
  return {text:d > 0 ? '▲ Mejorando' : d < 0 ? '▼ Atención' : '■ Estable', cls:d > 0 ? 'green' : d < 0 ? 'red' : 'neutral'};
}
function resolveStore(){
  const raw = $('storeSearch').value.trim();
  const q = raw.toLowerCase();
  if(!q) return;
  const f = dir.find(d => d.ceco === q || (`${d.ceco} · ${d.tienda}`.toLowerCase() === q) || d.tienda.toLowerCase() === q)
...
function clsScore(sc){ if(sc == null) return 'neutral'; return sc >= .75 ? 'green' : sc >= .5 ? 'neutral' : 'red'; }
...
function storeMetric(c,k,n,w){
  ...
  const promHtml = k === 'iplh'
    ? `<div class="split"><div class="splitRow"><span>IPLH</span><b class="value ${cls('iplh',avg(w.map(ww=>val(c,ww,'iplh'))))}">${fmt(avg(w.map(ww=>val(c,ww,'iplh'))),'num1')}</b></div><div class="splitRow"><span>TPLH</span><b class="value neutral">${fmt(avg(w.map(ww=>val(c,ww,'tplh'))),'num1')}</b></div></div>`
    : fmt(prom,def.fmt);
